...
def names():
    ...
    for num in range(names_length):
        ...
        total = acc + first_length

        for i in range(total):
            second_list = first_split[i]
            second_initial = second_list[0]

            space_initial = second_initial
            print(space_initial, end="")


def thirds():
    number_sentence = eval(input("enter the number of sentences:"))

    for num in range(number_sentence):
        num = num + 1
        sentence = input("enter the sentence")

        # set up range of print
